...
TEST(Point, dotProduct)
{
	Point<float> a{ 0, 0, 1 };
	Point<float> b{ 1, 0, 0 };
	ASSERT_EQ(Point<float>::dot(a, b), 0);
	Point<float> c{ 1.1, 2.2, 3.3 };
	Point<float> d{ 2.1, 2.2, 2.3 };
	ASSERT_FLOAT_EQ(Point<float>::dot(c, d), 14.74);
}
TEST(Point, cross)
{
	Point<double> a{ 0, 0, 1 };
	Point<double> b{ 1, 0, 0 };
	ASSERT_EQ(Point<double>::cross(a, b), Point<double>(0, 1, 0));
	Point<double> c{ 1.1, 2.2, 3.3 };
	Point<double> d{ 2.1, 2.2, 2.3 };
	ASSERT_EQ(Point<double>::cross(c, d).equal(Point<double>(-2.2, 4.4, -2.2)) , true);
}
TEST(Point, project)
{
