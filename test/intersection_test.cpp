...
TEST(hello, hello) { ASSERT_EQ(10, 10); }
TEST(interval, basic) {
...
TEST(Vector, cross) {
  ...
  ASSERT_EQ(Vector<double>::cross(c, d).equal(Point<double>(-2.2, 4.4, -2.2)),
            true);
}
TEST(Vector, project) {
...
TEST(trianle, intersecton) {
  std::array<Point<float>, 3> points1 = {
      Point<float>(1, 0, 0), Point<float>(0, 1, 0), Point<float>(0, 0, 1)};
  Convex<float, 3> tr1(points1);
  std::array<Point<float>, 3> points2 = {
      Point<float>(0, 0, 6), Point<float>(0, 0.5, 0), Point<float>(0, 0, 0)};
  Convex<float, 3> tr2(points2);
  ASSERT_EQ(Convex<float>::isIntersect(tr1, tr2), true);
  ASSERT_EQ(Convex<float>::isIntersect(tr2, tr1), true);
  std::array<Point<double>, 3> points3 = {
      Point<double>(1, 0, 0), Point<double>(0, 1, 0), Point<double>(0, 0, 1)};
  Convex<double, 3> tr3(points3);
  std::array<Point<double>, 3> points4 = {
      Point<double>(2, 0, 0), Point<double>(0, 2, 0), Point<double>(0, 0, 2)};
