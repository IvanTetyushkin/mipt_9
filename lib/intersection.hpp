...
#include <iostream>
namespace handmade {
template <typename T> bool almostEqual(T a, T b, T pol = 1e-3) {
...
public:
  Interval(T l = NAN, T r = NAN) : left{l}, right{r} {};
...
public:
  ...
  [[nodiscard]] bool equal(const Point &other) const {
...
template <typename T> class Vector final : public Point<T> {
  using PointT = Point<T>;

public:
  Vector(const PointT &from, const PointT &to) : Point<T>(to - from){};
  Vector(const PointT &p) : Point<T>(p){};
  ...
  static Vector cross(const Vector &p1, const Vector &p2) {
    return Vector{p1.y * p2.z - p1.z * p2.y, p1.z * p2.x - p1.x * p2.z,
                  p1.x * p2.y - p1.y * p2.x};
  }
  [[nodiscard]] T getLen() const { return std::sqrt(dot(*this, *this)); }
  // not need
#if 0
	Vector& normilize()
	{
  ...
  static int isParallel(const Vector &lhs, const Vector &rhs) {
    ...
    T tmp_coeff = x_coeff != 0   ? x_coeff
                  : y_coeff != 0 ? y_coeff
                                 : z_coeff; // well one of them not null
  ...
  [[nodiscard]] T project(const Vector &p) const { return dot(*this, p); }
};
template <typename T, size_t size = 3> class Convex {
  ...
  [[nodiscard]] IntervalT getMinMaxProjection(const VectorT &v) const {
  ...
  [[nodiscard]] VectorT getEdge(int i) const {
  ...
  static bool checkAx(const Convex<T, size1> &lhs, const Convex<T, size2> &rhs,
                      const VectorT &ax) {
    IntervalT lhs_proj = lhs.getMinMaxProjection(ax);
    IntervalT rhs_proj = rhs.getMinMaxProjection(ax);
    return static_cast<bool>(IntervalT::areNotIntersect(lhs_proj, rhs_proj));
  }

public:
  Convex(const std::array<Point<T>, 3> &ps) : Points{ps} { validate(); };
  // https://www.geometrictools.com/Documentation/MethodOfSeparatingAxes.pdf
  template <size_t size1, size_t size2>
  static bool isIntersect(const Convex<T, size1> &lhs,
                          const Convex<T, size2> &rhs) {
