...
private:
	...
	std::unordered_map<AddrType, listIter> _hash;//store address -> iterator to value
...
public:
	...
	{
		auto findResult = std::find_if(std::begin(_hash), 
			std::end(_hash),
	...
	std::tuple<result, StoreType> getElement(const AddrType& addr) override final
	{
		...
		if (searchRes == _hash.end())
		{// no such address stored
			if (isFull())
				removeOldElement();
	...
	void deleteElement(const AddrType& toDel) override final
	{
		listIter pos = _hash.at(toDel);
		_hash.erase(toDel);
