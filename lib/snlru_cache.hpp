...
private:
  ...
  void swapCacheElements(size_t to, size_t from, const AddrType &addr) {
    ...
    assert(_cache.at(from).checkElement(addr).has_value() &&
           "error try to swap nonexist");
...
public:
  snlru_cache(size_t sz) : _cache{sz, sz, sz} {};
  ...
  std::tuple<result, StoreType> getElement(const AddrType &addr) final {
    if (_cache[0].checkElement(addr).has_value()) { // it is in hot cache
      return _cache[0].getElement(addr);
    }
    for (int i = 1; i < 3; ++i) {
      if (_cache[i].checkElement(addr).has_value()) { // it is in cache
  ...
  void deleteElement(const AddrType &addr) final {
    for (int i = 0; i < 3; ++i) {
      if (_cache[i].checkElement(addr).has_value()) { // it is in cache
